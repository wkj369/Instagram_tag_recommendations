...
def upload():
    if request.method == 'POST':
        ...
        unique_list_kr, tokenized_doc, tok_list = yoloModel(file)
        keyword = Myword2vec(tok_list)
        lda_json = lda_model(keyword, tokenized_doc)
        return render_template("index.html", before_image=file, display=display, lda_json=lda_json, keyword=keyword)
